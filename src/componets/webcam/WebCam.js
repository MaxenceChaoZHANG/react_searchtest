...
import {Button,Row, Col, Select, message, Radio, Tag,Modal}  from "antd";
...
export default class WebCamWebAPI extends React.Component{
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            mediaStream : null,
            deviceActive : false,
            fileTypes : [],//TYPE_CODE,TYPE_DESC
            fileTypeSeq : 0,//当前照片类型 TYPECODE>0 
            addFinish : null,//每个类型照片数目
            tips : ""
        };
    }
    ...
    render() {
        ...
        return ( <div style={{backgroundColor: "#FFF", padding: '20px'}}>
            <Row>
                <Col span={12} key={1}>
                    <h3>实时画面:</h3>
                    ...
                    <Row>
                        ...
                        <Radio.Group value={this.state.fileTypeSeq} onChange={(e) => {
                            ...
                            {this.state.fileTypes.length > 0 ? this.state.fileTypes.map((item, index) => {
                                return (<Radio key={index} style={radioStyle} value={item.TYPE_CODE}>
                                        {this.state.addFinish[item.TYPE_CODE] > 0 ?
                                        <span>{`${item.TYPE_DESC} `}<Tag color='#52c41a'>{`已添加 ${this.state.addFinish[item.TYPE_CODE]} 张`}</Tag></span> 
                                        : item.TYPE_DESC}</Radio>);
                            }) : null}
                        </Radio.Group>
                    </Row>
                 </Col>
